    ...
    network.forEachStructure((struct, structId) => {
      if (knownStructureIds.has(structId)) {
        ...
        if (mesh) {
          ...
          // DO NOT touch mesh.scale — it's set correctly on load.
          // Setting scale.y here overrides the model's height normalization
          // (model.scale.setScalar(targetHeight/size.y)) and causes the
          // structure to appear cut in half.
